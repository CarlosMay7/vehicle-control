/**
 * @file VehicleIndex.jsx
 * @description Vista de listado para la gestión de Vehículos.
 * Este componente es responsable de obtener y mostrar una tabla con la información
 * de todos los vehículos registrados en el sistema. Utiliza el componente
...
export const VehicleIndex = () => {
  const [data, setData] = useState({}); // Almacena los encabezados y filas de la tabla de vehículos.
   ...
   * @description Realiza una llamada a la API para obtener la lista de vehículos.
   * Procesa los datos recibidos para adaptarlos al formato requerido por `GenericIndex`.
   * @returns {Promise<Array<Array<any>>>} Un array de arrays, donde cada sub-array es una fila de datos del vehículo.
   */
  async function getVehicles() {
    try {
      ...
      if (!res.ok) throw new Error("Failed to fetch vehicles"); // Manejo de errores de la API.
      const data = await res.json();

      // Mapea los datos crudos de la API a un formato de fila adecuado para `GenericIndex`.
      const rows = data.map((v) => [
        v.id, // ID del vehículo (oculto en la tabla, usado para acciones).
        v.brand,
        v.model,
        v.vin, // Número de identificación del vehículo.
        v.licensePlate, // Matrícula.
      ]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const values = await getVehicles(); // Obtiene los datos de los vehículos.
      setData({
        headers: ["Brand", "Model", "VIN", "License Plate"], // Define los encabezados de la tabla.
  ...
  return (
    <>
      ...
      {/* Muestra un mensaje de carga o el índice de vehículos. */}
